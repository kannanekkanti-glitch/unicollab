      ...
      <div className="sticky top-20 space-y-5">
        ...
        {user && (
          <div className="p-4 rounded-2xl glass-card transition-all duration-300 hover:shadow-xl hover:border-brand-500/30">
            ...
            <div className="mt-3 pt-3 border-t border-slate-100 dark:border-white/10 flex items-center justify-between text-[11px]">
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `relative flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-medium transition-all duration-200 group ${
                    isActive
                      ? 'bg-gradient-to-r from-brand-500/15 via-indigo-500/10 to-transparent text-brand-700 dark:text-brand-300 font-bold shadow-sm'
              ...
              >
                {({ isActive }) => (
                  <>
                    {isActive && (
                      <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-brand-500 rounded-r-full shadow-sm shadow-brand-500" />
                    )}
                    <div className="flex items-center gap-3">
                      <Icon className={`w-4 h-4 transition-transform duration-200 group-hover:scale-110 ${
                        isActive ? 'text-brand-600 dark:text-brand-400' : 'text-slate-400 group-hover:text-brand-500'
                      }`} />
        ...
        <div className="p-4 rounded-2xl bg-gradient-to-br from-indigo-500/15 via-brand-500/10 to-violet-500/15 border border-brand-500/30 text-xs shadow-md shadow-brand-500/5">
          <h4 className="font-bold text-brand-900 dark:text-brand-200 flex items-center gap-1.5">
            <Shield className="w-3.5 h-3.5 text-brand-600 dark:text-brand-400" />
            Verified Campus Space
          </h4>
          <p className="text-[11px] text-slate-600 dark:text-slate-300/90 mt-1 leading-relaxed">
